...
namespace restapi.Controllers
{
    ...
    public class TimesheetsController : Controller
    {
        ...
        [HttpGet("{id}/lines/lineId")]
        [Produces(ContentTypes.TimesheetLines)]
        [ProducesResponseType(typeof(IEnumerable<AnnotatedTimecardLine>), 200)]
        [ProducesResponseType(404)]
        public IActionResult GetLineId(string id, string lineId)
        {
            Timecard timecard = Database.Find(id);

            if (timecard != null)
            {
                int index = timecard.searchLine(lineId); 

                if(index >= 0){
                    AnnotatedTimecardLine line = timecard.Lines[index]; 
                    return Ok(line);
                }else{
                    return NotFound();
                }
        ...
        public IActionResult ReplaceLine(string timecardId, string lineId, [FromBody] TimecardLine timecardLine)
        {
            ...
            else
            {
                ...
                return Ok(timecard.replace(index, timecardLine));
            }
        ...
        [ProducesResponseType(typeof(IEnumerable<AnnotatedTimecardLine>), 200)]
        [ProducesResponseType(typeof(InvalidStateError), 409)]
        public IActionResult UpdateLines(string timecardId, string lineId, [FromBody] TimecardLine timecardLine)
        {
            Timecard timecard = Database.Find(timecardId);
            ...
            else
            {
                ...
                return Ok(timecard.update(index, timecardLine));
            }
